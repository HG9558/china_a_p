    ...
    <div class="px-3 bg-white pt-1" v-show="data.hshow === 'Yes'">
        ...
        <ul class="text-sm pb-10px" v-show="!data.isHide">
            <li
                ...
                class="grid grid-cols-3 text-center items-center gap-2 text-mainText font-medium"
                :class="{ '!grid-cols-2': data.title?.length == 2 }"
            >
                <div
                    :key="index"
                    class="line-clamp-1 h-9 leading-9"
                    v-for="(item, index) in data.title"
                    :class="{ 'flex items-center': isSame(details, item.osn, data) }"
                >
                    <img
...
import type { Datum, HP } from "@/api/obsportend/obMatches/types";
