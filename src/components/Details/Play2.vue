        ...
        <ul class="text-sm pb-10px" v-show="!data.isHide">
            ...
            >
                <div class="line-clamp-1" v-for="(item, index) in data.title" :key="index">
                    <div :class="{ 'flex justify-between': isSame(details, item.osn, data) }">
                        <img
                            class="w-[18px] h-[18px] mr-[6px] flex-shrink-0"
                            v-show="isSame(details, item.osn, data) && index == 0"
                        ...
                        <p
                            ...
                            :class="{
                                'text-left': index == 0 && /(2|3)/.test(`${data.title?.length}`),
                                'text-center': index == 1 && data.title?.length == 3,
                                'text-right':
                                    (index == 2 && data.title?.length == 3) ||
                                    (index == 1 && data.title?.length == 2),
                            }"
                        >
                            {{ item.osn }}
                        </p>
                        <img
                            class="w-[18px] h-[18px] ml-[6px] flex-shrink-0"
                            v-show="isSame(details, item.osn, data) && index == 1"
                            v-lazy="isSame(details, item.osn, data)"
                            alt=""
                        />
                    </div>
...
import { betActive, isSame, matchStatus } from "@/utils/match";
import { hpid } from "@/utils/match";
